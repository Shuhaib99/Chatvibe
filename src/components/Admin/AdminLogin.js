    ...
    <div className='h-screen bg-black flex justify-center  z-50 text-white '>
      ...
      <div className='flex justify-center items-center '>
        <div>
          ...
          <input value={email} className='w-80 p-2 px-3 mt-5 bg-transparent border border-white outline-none rounded-3xl text-yellow-400' type="text" onChange={(e) => {
            setEmail(e.target.value)
            setErrEmail(false)
          }} placeholder='Info@example.com' />
          <span style={{ display: !errEmail ? "none" : "block", color: "red", fontSize: "12px" }}>Please enter valid email</span>
          <br />
          <input value={password} className='w-80 p-2 px-3 mt-5 bg-transparent border border-white outline-none rounded-3xl text-yellow-400' type="password" onChange={(e) => {
          ...
          <button
            className='w-80 py-2 bg-transparent border border-white text-white hover:text-yellow-400 duration-300'
            type='submit'
            onClick={() => {
              handleLogin()
            }} >
